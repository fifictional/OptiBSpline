...
def bspline_objective(control_points, target_points, degree, knot_vector,
                     n_samples=3500, alpha=500.0, beta=1.0, gamma=500000.0, delta=1000.0):
    control_points = control_points.to(device).to(DTYPE)
...
# torch.save(opt_curve.cpu(), 'optibspline_sphere.pt')
# opt_curve = torch.load('optibspline_sphere.pt').to(device)




# # --- resampling - more points where the bends are sharper ---
opt_np = opt_curve.detach().cpu().numpy()  
vel = np.diff(opt_np, axis=0)               
accel = np.diff(vel, axis=0)               
curvature = np.linalg.norm(accel, axis=1) / (np.linalg.norm(vel[:-1], axis=1) + 1e-6)
if len(curvature) >= 1:
    curvature = np.concatenate(([curvature[0]], curvature, [curvature[-1]]))
else:
    curvature = np.zeros(len(opt_np))

weights = 1.0 + 5.0 * curvature   
t_values = np.cumsum(np.concatenate(([0.0], weights[:-1])))   
t_values = t_values / t_values[-1]                            

new_t = np.linspace(0.0, 1.0, len(opt_np))
resampled = np.zeros_like(opt_np)
for i in range(opt_np.shape[1]):
    resampled[:, i] = np.interp(new_t, t_values, opt_np[:, i])

opt_curve = torch.tensor(resampled, dtype=torch.float32, device=opt_curve.device)
# ----------
